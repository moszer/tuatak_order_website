...
export async function GET(_req: NextRequest) {
  ...
  const [rows] = await pool.query(
    `SELECT c.id, c.code, c.title, c.description, c.discount_type, c.discount_value,
            c.min_order, c.points_cost, c.expires_at, c.max_uses, c.used_count, c.per_member_uses,
            COUNT(mc.id) as owned_count,
            MIN(mc.id) as mc_id,
            MAX(mc.claimed_at) as claimed_at,
            MAX(mc.points_spent) as points_spent
     FROM member_coupons mc
     JOIN coupons c ON mc.coupon_id = c.id
     WHERE mc.member_id = ? AND mc.is_used = 0
